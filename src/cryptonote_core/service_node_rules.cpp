...
namespace service_nodes {
	...
	uint64_t get_staking_requirement(cryptonote::network_type m_nettype, uint64_t height)
	{
		...
		{
			base = 70000 * COIN;
			variable = (20000.0 * COIN) / italo::exp2(height_adjusted / 356446.0);
		}
		else
		{
			base = 10000 * COIN;
			variable = (30000.0 * COIN) / italo::exp2(height_adjusted / 129600.0);
		}
...
uint64_t get_portions_to_make_amount(uint64_t staking_requirement, uint64_t amount)
{
  ...
  div128_64(hi, lo, staking_requirement, &resulthi, &resultlo, NULL, NULL);
  return resultlo;
}
